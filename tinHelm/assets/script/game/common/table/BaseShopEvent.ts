...
/**
 * BaseShopEvent 派生类构造类型。
 */
interface BaseShopEventConstructor<T extends BaseShopEvent> {
    new (): T;
    TableName: string;
    TableId: number;
 ...
 * 自动生成文件，请勿手动修改。
 */
export class BaseShopEvent {

    /** JsonUtil 中的配置表名称 */
    static TableName: string = "14_ShopEvent";
    static readonly TableId: number = 14;
    ...
    /** 当前配置原始数据 */
    private data: ShopEventConfigData = null!;
    ...
    static createId(localId: number): number {
        if (!Number.isInteger(localId) || localId <= 0 || localId >= this.LocalIdBase) {
            throw new Error(
                "ShopEvent 表内ID必须在0001到9999之间，当前值：" + localId
            );
        }
     ...
     * 获取全部配置。
     */
    static getAllConfig<T extends BaseShopEvent>(
        this: BaseShopEventConstructor<T>
    ): T[] {
        const table = JsonUtil.get(
            this.TableName
        ) as Record<string, ShopEventConfigData> | null;
     ...
     * 不存在时返回 null。
     */
    static getConfigById<T extends BaseShopEvent>(
        this: BaseShopEventConstructor<T>,
        id: number
    ): T | null {
        if (!this.isOwnId(id)) {
