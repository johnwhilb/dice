...
/**
 * BaseUniversal 派生类构造类型。
 */
interface BaseUniversalConstructor<T extends BaseUniversal> {
    new (): T;
    TableName: string;
    TableId: number;
 ...
 * 自动生成文件，请勿手动修改。
 */
export class BaseUniversal {

    /** JsonUtil 中的配置表名称 */
    static TableName: string = "12_Universal";
    static readonly TableId: number = 12;
    ...
    /** 当前配置原始数据 */
    private data: UniversalConfigData = null!;
    ...
    static createId(localId: number): number {
        if (!Number.isInteger(localId) || localId <= 0 || localId >= this.LocalIdBase) {
            throw new Error(
                "Universal 表内ID必须在0001到9999之间，当前值：" + localId
            );
        }
     ...
     * 获取全部配置。
     */
    static getAllConfig<T extends BaseUniversal>(
        this: BaseUniversalConstructor<T>
    ): T[] {
        const table = JsonUtil.get(
            this.TableName
        ) as Record<string, UniversalConfigData> | null;
     ...
     * 不存在时返回 null。
     */
    static getConfigById<T extends BaseUniversal>(
        this: BaseUniversalConstructor<T>,
        id: number
    ): T | null {
        if (!this.isOwnId(id)) {
