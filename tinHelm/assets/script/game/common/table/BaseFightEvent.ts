...
/**
 * BaseFightEvent 派生类构造类型。
 */
interface BaseFightEventConstructor<T extends BaseFightEvent> {
    new (): T;
    TableName: string;
    TableId: number;
 ...
 * 自动生成文件，请勿手动修改。
 */
export class BaseFightEvent {

    /** JsonUtil 中的配置表名称 */
    static TableName: string = "15_FightEvent";
    static readonly TableId: number = 15;
    ...
    /** 当前配置原始数据 */
    private data: FightEventConfigData = null!;
    ...
    static createId(localId: number): number {
        if (!Number.isInteger(localId) || localId <= 0 || localId >= this.LocalIdBase) {
            throw new Error(
                "FightEvent 表内ID必须在0001到9999之间，当前值：" + localId
            );
        }
     ...
     * 获取全部配置。
     */
    static getAllConfig<T extends BaseFightEvent>(
        this: BaseFightEventConstructor<T>
    ): T[] {
        const table = JsonUtil.get(
            this.TableName
        ) as Record<string, FightEventConfigData> | null;
     ...
     * 不存在时返回 null。
     */
    static getConfigById<T extends BaseFightEvent>(
        this: BaseFightEventConstructor<T>,
        id: number
    ): T | null {
        if (!this.isOwnId(id)) {
