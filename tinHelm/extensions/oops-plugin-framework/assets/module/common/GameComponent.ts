...
export class GameComponent extends Component {
    ...
    /** Sprite 当前占用的资源及最后一次异步加载请求 */
    private spriteResRecords = new Map<Sprite, SpriteResRecord>();
    ...
    async load<T extends Asset>(bundleName: string, paths: Paths | AssetType<T>, type?: AssetType<T>): Promise<T> {
        ...
        try {
            ...
            // 只记录已经成功 addRef 的资源。组件在加载期间销毁时立即归还引用。
            if (!isValid(this)) {
                oops.res.release(realPath, realBundle);
                return null!;
            }
    ...
    async setSprite(target: Sprite, path: string, bundle: string = resLoader.defaultBundleName): Promise<boolean> {
        ...
        const previous = this.spriteResRecords.get(target);
        this.spriteResRecords.set(target, {
            path: previous?.path ?? '',
        ...
        // 加载结束到赋值之间组件仍可能被销毁，资源已由 onDestroy 统一释放。
        if (!isValid(this)) {
            return false;
        }

        const current = this.spriteResRecords.get(target);
        if (!isValid(target) || current?.requestId !== requestId) {
        ...
        if (current.path) {
            this.releaseRes(current.path, current.bundle);
        }
        this.spriteResRecords.set(target, { path, bundle, requestId });
        return true;
    }
    //#endregion
    ...
    protected onDestroy() {
        ...
        // SpriteFrame 必须先从渲染组件解绑，再释放对应资源。
        this.spriteResRecords.forEach((_record, sprite) => {
            if (isValid(sprite)) {
                sprite.spriteFrame = null;
            }
        });
        this.spriteResRecords.clear();
