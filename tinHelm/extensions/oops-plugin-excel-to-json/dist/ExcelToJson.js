...
function getFieldEnums(worksheet, name) {
    const fields = [];
    const keys = new Set();
    ...
    worksheet.getRow(2).eachCell((cell, column) => {
        ...
        fields.push({ name: enumName, column, members: new Map() });
    });
    return fields;
}
/** 将字段中的英文成员名映射到同一行的配置主键。 */
function collectFieldEnums(fields, worksheet, rowNumber, id) {
...
function writeFieldEnums(fields) {
    const outputPath = path_1.default.join(__dirname, main_1.config.PathTsClient.replace("project://", "../../../"));
    for (const field of fields) {
        const members = Array.from(field.members, ([memberName, id]) => {
            return `    ${memberName} = ${id},`;
        }).join("\n");
        const script = `/**\n * 自动生成文件，请勿手动修改\n */\nexport enum ${field.name} {\n${members}\n}\n`;
        fs.writeFileSync(path_1.default.join(outputPath, `${field.name}.ts`), script);
 ...
 */
async function createLegacyEnumTs(workbook, name) {
