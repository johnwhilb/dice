...
async function createTsClient(name, fieldType, _data, primary) {
    ...
    if (fieldType[primaryKey] == null) {
        console.warn(`配置表【${name}】主键字段【${primaryKey}】没有客户端类型信息`);
        return;
    }
    let interfaceFields = "";
...
`;
    ...
    const baseScript = `import { JsonUtil } from "db://oops-framework/core/utils/JsonUtil";
...
${getters}}
`;
    const tableScript = `import { Base${name} } from "./Base${name}";
...
`;
    ...
    fs.writeFileSync(basePath, baseScript);
    console.log(`配置基类【Base${name}】生成成功`);
    if (!fs.existsSync(tablePath)) {
        fs.writeFileSync(tablePath, tableScript);
        console.log(`配置扩展【Table${name}】生成成功`);
    }
    else {
        console.log(`配置扩展【Table${name}】已存在，跳过生成`);
    }
}
 ...
 */
async function createTsServer(name, fieldType, _data, primary) {
    ...
    scriptInitParams = scriptInitParams.substring(0, scriptInitParams.length - 2);
    scriptInitVar = scriptInitVar.substring(0, scriptInitVar.length - 5);
    scriptInitValue = scriptInitValue.substring(0, scriptInitValue.length - 9);
    let field = "";
...
`;
    const p = path_1.default.join(__dirname, main_1.config.PathTsServer.replace("project://", "../../../") + "/");
    fs.writeFileSync(`${p}Table${name}.ts`, script);
}
